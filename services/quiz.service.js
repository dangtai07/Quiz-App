...
class QuizService {
    async createQuiz(quizData, files) {
        try {
            ...
            // NOTE: quiz.number will be auto-generated by pre-save middleware
            const quiz = new Quiz({
            ...
            await quiz.save();
            console.log(`✅ Quiz "${quiz.title}" (Number: ${quiz.number}) created successfully for ${quiz.roomCode.toUpperCase()} department with ${processedQuestions.length} questions`);
            return quiz;
    ...
    async updateQuiz(id, quizData, files) {
        try {
            ...
            console.log(`✅ Quiz "${updatedQuiz.title}" (Number: ${updatedQuiz.number}) updated successfully in ${roomCode?.toUpperCase()} department`);
            return updatedQuiz;
    ...
    async deleteQuiz(id) {
        try {
            ...
            console.log(`✅ Quiz "${quiz.title}" (Number: ${quiz.number}) deleted successfully from ${quiz.roomCode?.toUpperCase()} department`);
    ...
    // NEW: Helper method to find gaps in quiz numbers
    ...
    async searchQuizzes(searchParams) {
        try {
            const { 
                ...
                minQuestions,
                maxQuestions,
                roomCode, // Room code filter
                minNumber, // NEW: Quiz number filters
                maxNumber,
                sortBy = 'newest' 
            } = searchParams;
            ...
            // NEW: Filter by quiz number range
            ...
            switch (sortBy) {
                ...
                case 'name':
                    ...
                    break;
                case 'number':
                    sortQuery = { number: 1 }; // NEW: Sort by quiz number
                    break;
                case 'questions':
                ...
                case 'room':
                    ...
                    break;
                default:
                    sortQuery = { number: 1 }; // Default to quiz number sorting
            }
